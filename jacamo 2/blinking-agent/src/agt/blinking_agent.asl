...
/* Plans. */
    ...
    +!start 
        <- .println("Initializing...");             //internal method of the agent
           !createLampArtifact(lampId_0);           //triggering the plan "createLampArtifact" defined below
    ...
    +!createLampArtifact(I_Id)          //Create a lamp artifact with artifact id equals to "I_Id". The parameters of a plan may be either inputs or outputs (I_: input, O_: output)
        <-  .println("Creating lamp artifact...");
            ...
            makeArtifact(I_Id, "tools.LampArtifact", [I_Id, InitialState], LampRef);
    ...
    +state(LampState)                   //Triggered when an observable property called "state" in updated among the observed artifacts (in this case there's only one lamp)
        <- .println("Lamp state: ", LampState);
           ...
           if (LampState) { switchOff; } else { switchOn; }.     //If "the lamp is on" then "switch it off" else "switch it on"
